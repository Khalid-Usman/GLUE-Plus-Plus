...
def fit_SCGLUE(
    ...
    pretrain_init_kws.update({"shared_batches": False, "alignment_method": alignment_method})
    
    pretrain_compile_kws = compile_kws.copy()
    pretrain_compile_kws.update({"alignment_method": alignment_method})
    
    pretrain_fit_kws = fit_kws.copy()
    # In pretraining, align_burnin is infinity.
    # The SCGLUETrainer logic I wrote in scglue.py now checks this burnin variable
    # to disable OT (or Adversarial) during this phase.
    ...
    pretrain.compile(**pretrain_compile_kws)
    pretrain.fit(adatas, graph, **pretrain_fit_kws)
    if "directory" in pretrain_fit_kws:
        pretrain.save(os.path.join(pretrain_fit_kws["directory"], "pretrain.dill"))

    # Balancing logic usually applies only if we used adversarial alignment,
    # or if we want to weight samples based on preliminary embedding density.
    # It doesn't hurt to leave it enabled for OT as well if desired.
    if not skip_balance:
    ...
    finetune_compile_kws = compile_kws.copy()
    finetune_compile_kws.update({"alignment_method": alignment_method})
    
    finetune_init_kws = init_kws.copy()
    finetune_init_kws.update({"alignment_method": alignment_method})

    finetune = model(adatas, sorted(graph.nodes), **finetune_init_kws)
    finetune.adopt_pretrained_model(pretrain)
    finetune.compile(**finetune_compile_kws)
