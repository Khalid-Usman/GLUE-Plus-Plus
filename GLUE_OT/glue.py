...
class GLUE(torch.nn.Module):
    ...
    du
        Modality discriminator (optional, only needed for adversarial alignment)
    prior
        Latent prior
    """
    ...
    def __init__(
        ...
        self.du = du  # Can be None for OT alignment
        self.prior = prior
    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Only create discriminator optimizer for adversarial alignment
    ...
    def train_step(
        ...
        # Only run discriminator step for adversarial alignment
        if self.alignment_method == "adversarial" and self.dsc_optim is not None:
            # Discriminator step
    ...
    def fit(  # pylint: disable=arguments-renamed
        ...
        if reduce_lr_patience:
            # Only pass dsc_optim if it exists (adversarial alignment)
            if self.dsc_optim is not None:
                ...
                # OT alignment - only VAE optimizer
                default_plugins.append(
                    LRScheduler(
                        self.vae_optim,
                        monitor=self.earlystop_loss,
    ...
    def state_dict(self) -> Mapping[str, Any]:
        ...
        # Only save dsc_optim if it exists
        if self.dsc_optim is not None:
            state["dsc_optim"] = self.dsc_optim.state_dict()
        return state

    def load_state_dict(self, state_dict: Mapping[str, Any]) -> None:
        self.vae_optim.load_state_dict(state_dict.pop("vae_optim"))
        # Only load dsc_optim if it exists in both state_dict and current trainer
        if "dsc_optim" in state_dict and self.dsc_optim is not None:
            self.dsc_optim.load_state_dict(state_dict.pop("dsc_optim"))
        elif "dsc_optim" in state_dict:
            # State dict has it but current trainer doesn't - just pop it
            state_dict.pop("dsc_optim")
