...
import ot  # Python Optimal Transport library for Sinkhorn algorithm
...
class SCGLUETrainer(GLUETrainer):
    ...
    **kwargs
    ...
    def __init__(
        ...
        alignment_method: str = "adversarial",  # NEW: "adversarial" or "ot"
        ot_reg: float = 0.05,                   # NEW: Sinkhorn regularization
        ot_max_iter: int = 100,                 # NEW: Max Sinkhorn iterations
        ...
        **kwargs,
        ...
        if self.alignment_method not in ["adversarial", "ot"]:
            raise ValueError(
                f"alignment_method must be 'adversarial' or 'ot', "
                f"got '{self.alignment_method}'"
            )
        ...
        # Add OT loss to required losses if using OT
    ...
    def compute_losses(
        ...
        # ====================================================================
        # ALIGNMENT LOSS: Choose between Adversarial and Optimal Transport
        # ====================================================================
        
        if self.alignment_method == "ot":
            # ================================================================
            # OPTIMAL TRANSPORT ALIGNMENT using Sinkhorn Algorithm
            # ================================================================
            
            # Normalize embeddings for OT (CRITICAL for numerical stability)
            u_cat_norm = F.normalize(u_cat, dim=1)
            
            # Get modality masks (assuming binary: 0 and 1)
            modalities = torch.unique(xflag_cat)
            if len(modalities) != 2:
                raise ValueError(
                    f"OT alignment requires exactly 2 modalities, got {len(modalities)}"
                )
            
            mod0, mod1 = modalities[0].item(), modalities[1].item()
            mask0 = (xflag_cat == mod0)
            mask1 = (xflag_cat == mod1)
            ...
            if n0 == 0 or n1 == 0:
                ot_loss = torch.tensor(0.0, device=u_cat.device, requires_grad=True)
                alignment_loss = ot_loss
            else:
                # Extract modality-specific embeddings
                u0 = u_cat_norm[mask0]  # First modality (e.g., RNA)
                u1 = u_cat_norm[mask1]  # Second modality (e.g., ATAC)
                
                # Compute cost matrix (squared Euclidean distance)
                C = torch.cdist(u0, u1, p=2).pow(2)
                
                # Clamp to prevent numerical overflow
                C = torch.clamp(C, min=0, max=100)
                
                # Uniform probability distributions
                a = torch.ones(n0, device=u_cat.device) / n0
                b = torch.ones(n1, device=u_cat.device) / n1
                
                # Move to CPU for OT computation (POT library uses NumPy)
                ...
                try:
                    ...
                    if torch.isnan(ot_loss) or torch.isinf(ot_loss):
                        print(f"WARNING: Invalid OT loss (NaN/Inf), replacing with 0")
                        ot_loss = torch.tensor(0.0, device=u_cat.device, requires_grad=True)
                        
                except Exception as e:
                    print(f"ERROR in OT computation: {e}")
                    print(f"  n0={n0}, n1={n1}, C shape={C.shape}")
                    print(f"  C min={C.min().item():.4f}, max={C.max().item():.4f}")
                    ot_loss = torch.tensor(0.0, device=u_cat.device, requires_grad=True)
                
                alignment_loss = ot_loss
            
            # For compatibility with downstream code that expects dsc_loss
            dsc_loss = alignment_loss
            
        else:
            # ================================================================
            # ADVERSARIAL DISCRIMINATOR ALIGNMENT (Original GLUE)
            # ================================================================
            dsc_loss = F.cross_entropy(
                net.du(u_cat, xbch_cat), 
                xflag_cat, 
                reduction="none"
            )
            dsc_loss = (dsc_loss * xdwt_cat).sum() / xdwt_cat.numel()
            alignment_loss = dsc_loss
            ot_loss = torch.tensor(0.0, device=u_cat.device)  # For logging
        
        # Early return if only computing discriminator/alignment loss
        if dsc_only:
            if self.alignment_method == "ot":
                return {"dsc_loss": self.lam_align * alignment_loss, "ot_loss": ot_loss}
            else:
        ...
        gen_loss = vae_loss - self.lam_align * alignment_loss  # Use alignment_loss instead of dsc_loss
        ...
        # Add OT loss for logging if using OT
        if self.alignment_method == "ot":
    ...
    def train_step(
        ...
        if self.freeze_u:
            ...
            # Only train discriminator if using adversarial alignment
