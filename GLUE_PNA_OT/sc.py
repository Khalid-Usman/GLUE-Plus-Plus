...
CORRECTED VERSION - GraphEncoder adapted for new PNA implementation
"""
...
from ..num import EPS
from . import glue
from .nn import GraphConv  # Using the corrected GraphConv
...
class GraphEncoder(glue.GraphEncoder):
    r"""
    Graph encoder with PNA
    
    CORRECTED VERSION: Adapted to work with the new GraphConv that has
    learnable parameters and maintains dimensionality.
    ...
    use_degree_scalers
        Whether to use PNA degree scalers
    """
    ...
    def __init__(
        ...
        # CRITICAL: Use Xavier initialization, not zeros
        # This ensures gradients flow properly with max/min aggregators
        torch.nn.init.xavier_normal_(self.vrepr)
        
        # CORRECTED: GraphConv now takes input and output dimensions
        # and maintains dimensionality (out -> out), not expanding to 5*out
        self.conv = GraphConv(out_features, out_features, use_degree_scalers=use_degree_scalers)
        
        # CORRECTED: Linear layers now expect out_features (not 5*out_features)
        # because GraphConv maintains dimensionality through learned transformations
    ...
    def forward(
        ...
        # GraphConv now returns features of size out_features (not 5*out_features)
...
class GraphEncoderConcatenate(glue.GraphEncoder):
    r"""
    Alternative GraphEncoder using concatenation-based PNA.
    
    This maintains your original concatenation approach but with proper
    ...
    def __init__(self, vnum: int, out_features: int) -> None:
        ...
        from .nn import GraphConvConcatenate
        
        self.vrepr = torch.nn.Parameter(torch.zeros(vnum, out_features))
        torch.nn.init.xavier_normal_(self.vrepr)
        
        # GraphConvConcatenate handles the 5x expansion internally
        # and projects back to out_features
        self.conv = GraphConvConcatenate(out_features, out_features)
        
        # Linear layers expect out_features since GraphConvConcatenate
        # projects back to that dimension
        self.loc = torch.nn.Linear(out_features, out_features)
        self.std_lin = torch.nn.Linear(out_features, out_features)
